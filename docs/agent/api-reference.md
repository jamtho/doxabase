...
single-triple `rc:rowSemantics` adds with exact same-slot drift route to
`repair_or_replace` and include a `stage_map_assertion_change` replacement
suggestion that preserves `restages_revision`.
...
`suggested_next_calls` explaining why it is in the post-apply queue and how to
route it.
