...
Use `scanned_source_paths` for non-secret source files, object keys, or URI-like
inputs that should be recorded as `rc:DataSampleSource` spans. Use
`scanned_source_handles` for the reviewed file, object, table, or relation
handles that the external runtime actually scanned; handle-only values are
stored as `rc:scannedSourceHandle` metadata rather than source paths.
For a database handoff, a compact handle such as
`warehouse-prod:mart.orders` is acceptable when it is the reviewed
`connection_reference` plus `relation_identifier`. Query-context handoffs expose
both `scanned_source_paths` and `scanned_source_handles` separately.
...
reading per-item reasons. `status_counts` and `items[]` cover the full draft,
not only the accepted indexes for the current call. For a focused scalar
conflict option call, expect one selected option to be `staged` or `skipped`
and the remaining draft recommendations to appear as `not_selected`.
...
staged exports. The nested export also carries
`shareability_review_required=True` and
`shareability_review_status="required_not_completed"` even when scanner-clean.
Pass `fail_on_sensitive=True` to block before writing the nested
Markdown bundle when sensitive matches are present. Default discovery includes
...
rows are plain dicts in Python and JSON, so use
`group["review_lane"]` / `group["route_group_key"]` rather than dataclass
attribute access. The route keys can also come from profile route source
metadata persisted on a staged profile-map revision, or from generated
query-planning repair review notes for already-applied query repairs whose live
...
top-level `warnings` repeats that sibling readiness may now be stale when the
recheck list is non-empty. The top-level `suggested_next_actions` starts with
...
generated Markdown contains such matches. Staged Markdown export records also
set `shareability_review_required=True` and
`shareability_review_status="required_not_completed"` even when scanner-clean.
