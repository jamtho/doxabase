...
review. Both helpers call `describe_assertion_support`, generate the Turtle patch
payloads, and return the support snapshot plus a compact `judgement_panel`; the
draft helper also returns validation and impact previews without writing
`history`, while the staging helper links related lore and anchors on the staged
revision. Read the panel first when deciding whether a single assertion change is
plausible: it brings current/proposed values, value-type context, semantic risk
level/reasons, reasons the current value may be intentional, caveat scopes,
strongest routes, impact entries, and safety notes into one place. Use generic
...
`export_staged_revision` and `export_staged_revisions` actions generated by
staged apply, recovery, systematisation, profile-promotion, and staged-payload
search routes include `fail_on_sensitive=true` by default, while direct export
calls remain explicit. A staged patch with clean Turtle can still generate a
dirty review bundle when the live apply check reports drift, because that drift
summary is rendered from current graph terms at export time. After a broad
handoff or graph preflight blocks on unrelated project content, keep using
`fail_on_sensitive=true` for staged Markdown even when the selected staged row
looks clean. For simple
...
`informational` lane with `missing_patch_payload`. Inspect it with
`describe_graph_revision` and import the complete staged handoff if patch
payloads are expected. If explicit `revision_iris` include an applied revision
event, the planner returns an `inspect_already_applied` lane with
`applied_event_record` instead of sending that event through restage/apply
classification; inspect it with `describe_graph_revision` or
...
6. Run `restage_staged_revisions(revision_iris=plan.would_restage_revision_iris,
   dry_run=True)` and compare its classifications with the plan.
7. If the dry run still matches your review, run the real batch over the same
   worklist. Treat `restaged_revision_iris` as created successors, not as an
   apply queue.
8. For each candidate successor, call `check_staged_revision_apply()` and apply
   at most one `ready` row after review.
9. After any apply, rerun
...
10. For `repair_or_replace` / same-slot lanes, follow
