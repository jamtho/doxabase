...
def main() -> None:
    ...
    baseline = db.project_brief(limit=10)
    dataset_iri, evidence_iri = _record_synthetic_profile_run(db)
    after_profile = db.project_brief(limit=10)
    ...
    after_stage = db.project_brief(limit=10)
    ...
    print(f"Baseline profile queues: {{queue.name: queue.count for queue in baseline.queues}}")
    ...
    print(f"After-profile queues: {{queue.name: queue.count for queue in after_profile.queues}}")
    print(f"Staged profile revision: {staged_iri}")
    print(f"Staged recommendation indexes: {staged.staged_recommendation_indexes}")
    print(f"After-stage profile queues: {{queue.name: queue.count for queue in after_stage.queues}}")
